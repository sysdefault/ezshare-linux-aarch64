      ...
      <Section style={{ marginTop: 100 }}>
        ...
        <div style={{ margin: 'auto', maxWidth: 350, width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', marginBottom: 10 }}>
            <input type="text" onPaste={onPaste} placeholder="在此右键粘贴以发送剪贴板" style={{ display: 'block', width: '100%', boxSizing: 'border-box', textAlign: 'center', padding: '10px 0', border: '1px solid rgba(0,0,0,0.3)', fontFamily: 'inherit', fontSize: 15, borderRadius: 6 }} />
            <div style={{ whiteSpace: 'nowrap' }}><input id="saveAsFile" type="checkbox" onChange={(e) => setSaveAsFile(e.target.checked)} checked={saveAsFile} style={{ marginLeft: 10, verticalAlign: 'middle' }} /> <label htmlFor="saveAsFile" style={{ verticalAlign: 'middle' }}>保存为文件</label></div>
          </div>
            ...
            {clipboardText ? (
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
                  ...
                  <button onClick={() => setClipboardText()} type="button" style={{ padding: 5, flexGrow: 1 }}>取消</button>
                </div>
              </motion.div>
            ) : (
              <button onClick={onGetClipboard} type="button" style={{ padding: 10, width: '100%', boxSizing: 'border-box', backgroundColor: colorLink, border: 'none', borderRadius: 6, color: 'white', fontWeight: 'bold', fontSize: 17 }}>获取另一端剪贴板</button>
            )}
          </AnimatePresence>
        </div>
      </Section>
